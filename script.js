...
const shapes = [[[1,w+1,w*2+1,2],[w,w+1,w+2,w*2+2],[1,w+1,w*2+1,w*2],[w,w*2,w*2+1,w*2+2]],[[0,w,w+1,w*2+1],[w+1,w+2,w*2,w*2+1]],[[1,w,w+1,w+2],[1,w+1,w+2,w*2+1],[w,w+1,w+2,w*2+1],[1,w,w+1,w*2+1]],Array(4).fill([0,1,w,w+1]),[[1,w+1,w*2+1,w*3+1],[w,w+1,w+2,w+3]],
  ...
  [[1,w+1,w*2+1,w*2],[w,w+1,w+2,w*2+2],[1,w+1,w*2+1,2],[w,w*2,w*2+1,w*2+2]],
];
let pos=4, rot=0, idx=0, shape=[], color="", score=0, timer=null, blocks=[], settled=[];
function arraySome(arr, cb){
  for(let i=0; i<arr.length; i++) if(cb(arr[i],i,arr)) return true;
  return false;
}
function spawn(){
  pos=4; rot=0; idx=Math.floor(Math.random()*shapes.length);
  shape=shapes[idx][rot]; color=colors[idx]; blocks=[];
...
function render(){
  for(let j=0;j<shape.length;j++){
    let p=pos+shape[j], x=p%w, y=Math.floor(p/w);
    blocks[j].style.transform=`translate(${x*size}px,${y*size}px)`;
  }
}
function collide(offset=0){
  return arraySome(shape,i=>{
    let p=pos+i+offset, x=p%w, y=Math.floor(p/w);
    if(x<0||x>=w||y>=h) return true;
    for(let k=0;k<settled.length;k++) if(settled[k].x===x&&settled[k].y===y) return true;
    return false;
  });
}
function freeze(){
...
function clearRows(){
  ...
  for(let y=h-1; y>=0; y--){
    let row=[];
    for(let i=0; i<settled.length; i++) if(settled[i].y===y) row.push(settled[i]);
    if(row.length===w){
      ...
      for(let i=0; i<settled.length; i++){
        if(settled[i].y<y){
          settled[i].y++;
          settled[i].el.style.transform=`translate(${settled[i].x*size}px,${settled[i].y*size}px)`;
        }
      }
      rowsCleared++; y++;
    }
...
function rotate(){
  ...
  if(!arraySome(nextShape,i=>{
    let p=pos+i,x=p%w,y=Math.floor(p/w);
    if(x<0||x>=w||y>=h) return true;
    for(let k=0;k<settled.length;k++) if(settled[k].x===x&&settled[k].y===y) return true;
    return false;
  })){
    shape=nextShape; rot=nextRot; render();
  }
}
function gameOver(){
  clearInterval(timer);
...
for(let i=0; i<controls.length; i++){
  document.getElementById(controls[i].id).addEventListener("click", controls[i].handler);
}